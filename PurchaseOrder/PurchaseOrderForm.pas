...
uses
  ...
  cxDBLookupComboBox, cxLookAndFeelPainters, cxGroupBox, Buttons;
...
procedure TfrmPurchaseOrder.sbtPrintClick(Sender: TObject);
begin
  inherited;
  doPrintPreview(sender);
end;

procedure TfrmPurchaseOrder.doReportFilter(Sender: TObject);
begin
   ...
   Reportdm.JXNativeQuery.ParamByName('PDOCNO').Value :=dsMaster.dataset.FieldByName('order_no').AsString;
