...
type
  TAccountReceiveDM = class(TAbstractDM)
    ...
    tbReceiptHDREQUIRE_REFERENCE: TIBStringField;
    tbReceiptDTUNIT_COST: TFloatField;
    tbReceiptDTQTY: TFloatField;
    tbReceiptDTWH_AMOUNT: TFloatField;
    tbReceiptDTWH_RATE: TFloatField;
    qryOutstandingInvoice: TIBQuery;
    tbReceiptDTTAX_RATE: TFloatField;
    qryRevenue: TIBQuery;
    qryRevenuePURCHASETAX_AMOUNT: TIntegerField;
    qryRevenuePURCHASEBASE_AMOUNT: TFloatField;
    qryRevenueSALETAX_AMOUNT: TFloatField;
    qryRevenueSALEBASE_AMOUNT: TFloatField;
    qryRevenueREVENUE_AMOUNT: TFloatField;
    qryRevenueTAXDIFF_AMOUNT: TFloatField;
    ...
    procedure tbReceiptDTBASE_AMOUNTChange(Sender: TField);
    procedure tbReceiptDTWH_RATEChange(Sender: TField);
  private
    { Private declarations }
  public
     procedure updateBathText(docNo:string;mode:string);
...
procedure TAccountReceiveDM.updateBathText(docNo:string;mode:string);
  procedure handleException(e:exception);
  begin
    warning('Fail: update invoice header bath text'+e.Message);
  end;
  function getTrxAmount(docNo:string;mode:string):DOUBLE;
  begin
     result:=0;
     try
         if (mode ='INV')  then
...
begin
 updateAmount :=getTrxAmount(docNo,mode);
 if (updateAmount > 0) then
 begin
  {tbInvoiceHD.open;
  tbInvoiceHD.Edit;
  tbInvoiceHD.FieldByName('BATHTEXT').AsString:= TBahtStang.Create.NumericToBahtStang(updateAmount);
  tbInvoiceHD.Post;    }
  try
    qryAdhoc.close;
...
//  inherited;
  commit;
  updateBathText(dataset.fieldbyname('doc_no').AsString,'INV');


end;
procedure TAccountReceiveDM.updateLineInvoiceAmount(dataset:TDataSet) ;
begin
    if (tbInvoiceDTTAX_RATE.asFloat > 0) and (tbInvoiceDTBASE_AMT.AsFloat >0) then
    begin
...
begin
   ...
   begin
       ...
       begin
         ...
         FieldByName('ORDER_NO').AsString:='UNKNOWORD';
         FieldByName('ITEM_CODE').AsString:='DUMMY';
...
begin
//16-June-2015
 updateAmount  :=getTotalInvoiceAmount (tbReceiptHD.fieldByName('doc_no').AsString);
  if (updateAmount > 0) then
  begin
     tbReceiptHD.Edit;
     tbReceiptHD.FieldByName('BATHTEXT').AsString:=TBahtStang.Create.NumericToBahtStang(updateAmount)       ;
...
begin
  inherited;
  commit;
  updateBathText(dataset.fieldbyname('doc_no').asstring,'RV');
end;
...
procedure TAccountReceiveDM.tbReceiptDTBASE_AMOUNTChange(Sender: TField);
begin
  if (Sender.OldValue <> Sender.NewValue) then
  begin
    tbReceiptDTTAX_AMOUNT.AsFloat    :=  tbReceiptDTBASE_AMOUNT.AsFloat *tbReceiptDTTAX_RATE.AsFloat/100;
  end    ;
  if (tbReceiptDTWH_RATE.AsFloat > 0) then
       tbReceiptDTWH_AMOUNT.AsFloat :=  (tbReceiptDTBASE_AMOUNT.AsFloat *tbReceiptDTWH_RATE.asfloat/100)  ;
  tbReceiptDTTRX_AMOUNT.AsFloat :=  ( tbReceiptDTTAX_AMOUNT.AsFloat + tbReceiptDTBASE_AMOUNT.AsFloat) ;
end;

procedure TAccountReceiveDM.tbReceiptDTWH_RATEChange(Sender: TField);
begin
  IF (tbReceiptDTBASE_AMOUNT.AsFloat > 0 ) and (sender.Value >0) then
  begin
     tbReceiptDTWH_AMOUNT.AsFloat :=  (tbReceiptDTBASE_AMOUNT.AsFloat *sender.value/100)      ;
     tbReceiptDTBASE_AMOUNTChange(SENDER);
