...
uses
  ...
  cxTextEdit, cxGraphics, cxMaskEdit, cxDropDownEdit, jpeg,WCrypt2;
...
type
  TLoginDlg = class(TBaseDialog)
    ...
    Label3: TLabel;
    cbBusinessUnit: TcxComboBox;
    Label4: TLabel;
    cbConnection: TcxComboBox;
    Label6: TLabel;
    Panel1: TPanel;
    Panel2: TPanel;
    Image1: TImage;
...
implementation
uses WareHouseDM,mainForm,commonsharedatamodule;
...
begin
  ...
  floginTimeOut:=false;
  CenterForm(self);
  cbConnection.Properties.Items.Clear;
  cbConnection.Properties.Items.Add(WarehouseDataManager.getConnnectionString);
...
procedure TLoginDlg.cxButton3Click(Sender: TObject);
begin
 if CommonShareDM.changePassword(edUserId.Text,edPassword.Text) then
 begin
     ShowMessage('Succesful chagned password to login with new password');
 end
