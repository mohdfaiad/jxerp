...
{
18 July,2013 add supported password encrypt and decrypt
}
...
uses
  ...
  IBTable, IBQuery, IBStoredProc,Controls,abstractDataModule,jxcrypto;
...
type
 ...
 // property CurrentDatabase String read  databaseUrl write databaseUrl;
  ...
  function getConnnectionString:string;

     //reconnect database at any time
   function reConnect(databaseName:String):boolean;
    { Public declarations }
  end;
...
function TWarehouseDataManager.reConnect(databaseName:String):boolean;

  procedure handleError(e:exception);
  begin
      warning('Could not reconnect to'+databaseName+'with error ['+e.Message+']');
      result:=false;
  end;
begin
...
function TWarehouseDataManager.getConnnectionString:STRING;
BEGIN
...
begin
  ...
  qryAdhoc.SQL.Text:='SELECT user_name,default_Module  FROM SYS_USERS WHERE USER_ID='+
                     QuotedStr(user)+' and current_password='+quotedStr(EncryptStr(pwd,PASSWORD_KEY));
  qryAdhoc.open;
