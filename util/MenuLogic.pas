...
type TMatchedMenuItem     =class
    ...
    public
      MenuName,ModuleID,reportId:String;
      menuIndex,menuItemIndex:Integer;
      //report query string
      QueryString:String;
      constructor create(sysID,Caption:String;mIndex,mItemIndex:integer); overload;
      //another overoload constructor with reportId
      //24 Nov,2012
      constructor create(sysID,Caption:String;mIndex,mItemIndex:integer;mreportId:string); overload;
end;
...
type
  ...
  public
    ...
    function registerMenu(ModuleID:String;MenuLevelID,MenuItemIndex:Integer;menuName,reportid:string) :boolean;
...
{*** AR  Units****}
  ...
  APPaymentManagerForm, AbstractDataBrowse, TransactionType,
  CompanySetupForm, AROUTPUTVatReport,APPurchasevatreport,
  APInvoiceManagerForm,ARLedgerInquiry, AROutstandingReport;
...
constructor TMatchedMenuItem.create(SysID,Caption:String;mIndex,mItemIndex:integer;mreportId:string);
begin
   moduleID := sysID;
   menuName :=caption;
   menuIndex :=mIndex;
...
begin
  ...
  begin
     case matchedItem.menuIndex of
        ...
        1: case matchedItem.menuitemIndex of  //at transaction
               1:begin  //ar invice
                    InvoiceBrowseForm:= TInvoiceBrowseForm.create(self,'DOC_NO',matchedItem);
               ...
               2:begin {*** ar receipt***}
                    ReceiptBrowseForm := TReceiptBrowseForm.create(self,'DOC_NO',matchedItem);
        ...
        2: case matchedItem.menuitemIndex of  //report
           1:begin  //AR Ledger inquiry
                frmDebtorLedgerInquiry := TfrmDebtorLedgerInquiry.CREATE(SELF,matchedItem)  ;
                ArrangeUI(frmDebtorLedgerInquiry);
                frmDebtorLedgerInquiry.SHOW;
               result:=true;
            end;
            2:begin  //AROUTPUT VAT REOPROT
               frmAROutputVatReport := TfrmAROutputVatReport.create(self,matchedItem)  ;
               ArrangeUI(frmAROutputVatReport);
               frmAROutputVatReport.show;
               //AROUTPUT VAT REOPROT
               //('AR VAT IS coming soon');
              //  debug('AR VAT is coming soon')   ;
               result:=true;
            end;
              3:begin  //Outstanding
               frmAROutstandingReport := TfrmAROutstandingReport.create(self,matchedItem) ;
               ArrangeUI(frmAROutstandingReport);
               frmAROutstandingReport.Show;
               result:=true;
            end;
  ...
  begin
   case matchedItem.menuIndex of
        ...
        1: case matchedItem.menuitemIndex of
                1:begin //Quotation
                    frmCXSaleQuotaionManager := TfrmCXSaleQuotaionManager.create(SELF,'doc_no',matchedItem);
...
1	11	SOACT16	Sale Order Revision}
  ...
  begin
         case matchedItem.menuIndex of
              ...
              1: case matchedItem.menuitemIndex of
                      1:begin    //purchase order
                         frmPurchaseOrderBrowse := TfrmPurchaseOrderBrowse.Create(self,'ORDER_NO',matchedItem);
                      ...
                      2:begin //purchase order cancle
                         frmPurchaseOrderCloseCancelBrowse :=TfrmPurchaseOrderCloseCancelBrowse.create(SELF,'ORDER_NO',matchedItem);
                       ...
                       3:begin //purchase order cancle
                           if (GoodsReceiveNoteBrowseForm=nil) then
                           begin
                                GoodsReceiveNoteBrowseForm:=TGoodsReceiveNoteBrowseForm.create(self,'GRN_NO',matchedItem);
       ...
       else if   matchedItem.ModuleID = 'AP' then
       begin
          case matchedItem.menuIndex of
             0:case matchedItem.menuitemIndex of
                  4:begin //Supplier type
             ...
             END;
              1: case matchedItem.menuitemIndex of    // AP Payment
                 1: BEGIN
                    frmAPPaymentManager := TFrmAPPaymentManager.create(self,'DOC_NO',MatchedItem);
                    arrangeUI(frmAPPaymentManager);
                    frmAPPaymentManager.Show;
                    RESULT:=true;
                  END;
                  2: begin        //AP Invoice
                     frmAPInvoiceManager := TfrmAPInvoiceManager.create(self,'DOC_NO',MatchedItem);
                     arrangeUI(frmAPInvoiceManager);
                     frmAPInvoiceManager.Show;
                     result:=true;
                  end;
             end  ;
              2:case matchedItem.menuitemIndex of
                1:BEGIN
                   frmAPPurchaseVatReport := TfrmAPPurchaseVatReport.create(self,matchedItem);
                 //  arrangeUI(frmAPPurchaseVatReport);
                   frmAPPurchaseVatReport.show;
                   RESULT:=true;
                END;
             END;
          END;
...
begin
     ...
     begin
     qryAdhoc.Close;
     qryAdhoc.SQL.Text :=' select max(sys.module_ID) as moduleID, max(sys.menu_index)menuIndex,max(sys.menu_item_index)  menuItemIndex , MAX(SYS.REPORTID) AS REPORTID , max(sys.querystring) as queryString'+
                         ' from sys_menus sys, sys_user_menus usys  '+
                         ' where   sys.form_name = usys.form_name            '+
                         ' and sys.module_id = usys.module_id               '  +
                         ' and sys.menu_index = usys.menu_index            ' +
                         ' and sys.menu_item_index = usys.menu_item_index '  +
                         ' and usys.form_caption ='+QuotedStr(caption) +
                         ' and usys.user_id ='+quotedstr(WarehouseDataManager.User.userid) +
                         ' and usys.module_id='+QuotedStr(MODULE);
    ...
    begin
       qryAdhoc.Close;
       qryAdhoc.SQL.Text :=  'select max(sys.module_ID) as moduleID, max(sys.menu_index)menuIndex,max(sys.menu_item_index)  menuItemIndex, MAX(SYS.REPORTID) AS REPORTID , max(sys.querystring) as queryString'+
       //' select max(sys.module_ID) as moduleID, max(sys.menu_index)menuIndex,max(sys.menu_item_index)  menuItemIndex, MAX(SYS.REPORTID) AS REPORTID ,QueryString as ReportQuery'+
                         ' from sys_menus sys, sys_user_menus usys  '+
                         ' where   sys.form_name = usys.form_name            '+
                         ' and sys.module_id = usys.module_id               '  +
                         ' and sys.menu_index = usys.menu_index            '    +
    ...
    begin
       matchedItem :=  TMatchedMenuItem.create(dataset.FieldByName('moduleID').AsString,
                        ...
                        dataset.FieldByName('reportID').AsString);

       //set report query
       matchedItem.QueryString := dataset.FieldByName('queryString').asString;
...
function TLogicDM.registerMenu(ModuleID:String;MenuLevelID,menuItemIndex:Integer;menuName,reportid:string):boolean;
VAR cmdSQL:String ;
begin
   result:=false;
