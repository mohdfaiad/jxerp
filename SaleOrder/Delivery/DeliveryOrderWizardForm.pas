...
uses
  ...
  cxMemo, ActnList, XPStyleActnCtrls, ActnMan, Menus, DBGrids;
...
uses saleorderDatamodule,DeliveryORDERITEMLINKPOPUPForm;
...
begin
  result:=false;
  result :=(totalchargedWeight <=dsMelt.DataSet.FieldByName('postonhand').asfloat);
end;
...
begin
   ...
   begin
      ...
      if ( SaleOrderDataManager.deleteDeliveryOrderItem(
       ...
       end//delete delivery item success
       else
       begin
         debug('Process Abort');
       end;
...
begin
  SaleOrderDataManager.deleteDeliveryLinkByMeltNo(dsMelt.dataset.Fieldbyname('delivery_no').AsString,
