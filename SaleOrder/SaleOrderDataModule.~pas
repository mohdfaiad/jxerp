...
CONST STATE_APPROVED ='A';
CONST STATE_NORMAL ='N';
Type TInvoiceInfo = class

   public
   trxtype, invoiceno:String;
end;
...
type
  TSaleOrderDataManager = class(TAbstractDM)
    ...
    spCloneQuotation: TIBStoredProc;
  ...
  public
     ...
     //delete per lot no
     procedure deleteDeliveryLinkByMeltNo(deliveryNo:string; batchno:integer; meltno:string);
     ...
     function canPostDeliveryOrder(deliveryNo,custCode:string):boolean;
      {**
        13 December, 2012
      *to generate sale quotation to ar invice**}
     function SaleQuotationToInvoice  (Header:TDataSet;  invoice:TInvoiceInfo)  :boolean;


     {**
       23 April,2013 clone quotation
     **}
     function cloneSaleQuotation(DataSet:TDataSet;newDocNo:String ):boolean;
...
begin
 ...
 tbSaleQuoteDTTotal_AMT.AsFloat :=   tbSaleQuoteDTBASE_AMT.AsFloat+   tbSaleQuoteDTTAX_AMT.AsFloat;
...
procedure TSaleOrderDataManager.deleteDeliveryLinkByMeltNo(deliveryNo:string; batchno:integer; meltno:string);
...
begin
 try
      ...
      qryAdhoc.SQL.Text :='delete from so_det_dt_link' +#13   +
                         ...
                         ' and melt_no='+quotedStr(meltno)+#13   +
 ...
 with tbDeliveryDTlink do
  begin
       ...
       synchronizeDeliveryReservedQty(
        ...
        deleteDeliveryOrderItem( deliveryNo,custcode);
...
function TSaleOrderDataManager.SaleQuotationToInvoice  (Header:TDataSet;  invoice:TInvoiceInfo):boolean;
var HeaderSQL,DetailSQL:String;
    toBath:TBahtStang;
    done:boolean;
    procedure handleException(e:Exception);
    begin
...
begin
 try
   try
    toBath :=  TBahtStang.Create;
    result:=true;
    done:=false;
    HeaderSQL:='  INSERT INTO AR_INVOICE_HD  '+
                '  (DEBTOR_ACCT,   DOC_NO, DOC_DATE, '+
                '  TRX_TYPE,TAX_INV_NO, TAX_INV_DATE,  '+
                '  DEBT_NAME, REMARK, STATUS,  '+
                '  RECEIPT_STATUS,   DUE_DATE, CREDIT_TERM, '+
                '  NO_OF_PRINT,      UPDATE_USER,UPDATE_DATE, '+
                '  DESCS,     BATHTEXT,    CURRENCY_CODE,   CURRENCY_RATE,    REQUIRE_REFERENCE) '+
                '  SELECT    H.cust_code, '+ QuotedStr(invoice.invoiceno)+', CURRENT_DATE,'+
                QuotedStr(invoice.trxtype)+','+
                'H.DOC_NO,H.DOC_DATE, '+
                '  H.name,  H.remarks,' +QuotedStr(STATE_NORMAL)+','+
                QuotedStr(STATE_NORMAL)+','  +
                ' CURRENT_TIMESTAMP +H.term_of_payment ,H.term_of_payment, '+
                ' 0, '+
                QuotedStr(WarehouseDataManager.User.userid)+' ,CURRENT_TIMESTAMP, '+
                QuotedStr('Quotation to Invoice')+','+
                QuotedStr(toBath.NumericToBahtStang(header.fieldbyname('doc_amt').AsFloat+header.fieldbyname('tax_amt').AsFloat))+', '+
                '  h.currency_cd, '+
                '  1, '+QuotedStr('N')+
                ' FROM so_quotation_hd H  '+
                ' WHERE DOC_NO ='+ QuotedStr(header.FieldByName('DOC_NO').AsString);
                qryAdhoc.Close;
                qryAdhoc.sql.Text:=headerSQL;
                qryAdhoc.ExecSQL;                 
     except on e:exception do
            handleException(e);
     end;
     try
     DetailSQL:=' INSERT INTO AR_INVOICE_DT (    '+
              ' DEBTOR_ACCT, DOC_NO, DOC_DATE, TRX_TYPE,   '+
              ' DESCS, TAX_AMT, DOC_AMT,  '+
              ' BASE_AMT, ITEM_CODE,  '+
              ' ITEM_DESCS, QTY, UNIT_COST, UNIT_DESCS,    '+
              ' ORDER_NO, ORDER_DATE, TAX_RATE,   '+
              ' PURCHASE_NO, PURCHASE_DATE, '+
              ' DELIVERY_NO, DELIVERY_DATE, '+
              ' TH_BAHT, UPDATE_USER, UPDATE_DATE, LINE_NO)  '+
              ' select DT.cust_code , '+ QuotedStr(invoice.invoiceno)+', CURRENT_DATE,'+
              QuotedStr(invoice.trxtype)+','+
              ' DT.item_descs , DT.tax_amt , DT.total_amt, '+
              'DT.base_amt  ,DT.item_code, '+
              'DT.item_descs , DT.qty , DT.unit_cost, DT.item_descs,    '+
              'DT.doc_no , DT.doc_date,    dt.tax_rate,  '+
              QuotedStr(header.Fieldbyname('REF_NO').AsString)+','+
              QuotedStr(FormatDateTime('mm/dd/yyyy',header.FieldByName('REF_DATE').AsDateTime) )+','+
              'DT.doc_no , DT.doc_date,  '+QuotedStr('BathThai')+','+
              QuotedStr(WarehouseDataManager.User.userid)+' ,CURRENT_TIMESTAMP, 1'+
              ' FROM SO_QUOTATION_dt  DT '+
              ' WHERE DOC_NO ='+QuotedStr(header.FieldByName('DOC_NO').AsString);
             
              qryAdhoc.Close;
              qryAdhoc.sql.Text:=DetailSQL;
              qryAdhoc.ExecSQL;
              done :=true;
     except on e:exception do
            handleException(e);
     end;
     //final step update quotation as 'posted'
     try
           qryAdhoc.Close;
           qryAdhoc.SQL.Text :='UPDATE SO_QUOTATION_HD SET STATUS='+QuotedStr(STATE_APPROVED)+
                               ' WHERE DOC_NO='+QuotedStr(header.fieldbyname('DOC_NO').AsString);
           qryAdhoc.ExecSQL;
           done :=true;
     except on e:exception do
...
function TSaleOrderDataManager.cloneSaleQuotation(DataSet:TDataSet;newDocNo:String ):boolean;
   procedure handleException(e:Exception);
    begin
      result:=false;
    end;
begin
  ...
  try
     with dataset do
     begin
    spCloneQuotation.Close;
    spCloneQuotation.ParamByName('Adoc_no').value:=FieldByName('doc_no').AsString;
    spCloneQuotation.ParamByName('Adoc_date').value:=FieldByName('doc_Date').AsDateTime;
    spCloneQuotation.ParamByName('newdoc_no').value:=newDocNo;
    spCloneQuotation.ParamByName('newdoc_date').value:=Date; //CURRENT DATE
    spCloneQuotation.ParamByName('AUSERID').Value :=WarehouseDataManager.User.userid;
    spCloneQuotation.Prepare;
    spCloneQuotation.ExecProc;
    commit;
    end
  except on e:exception do
    handleException(e);
